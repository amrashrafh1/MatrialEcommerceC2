                        ...
                        <li class="list_var">
                            <div class="form-group">
                                <div class="col-md-12 row" id='list_0'>
                                    <div class='col m-1'>
                                    <input type='number' value='{{old('from', $rate->from)}}' name='from[]' placeholder='{{trans('admin.range_from')}}' class='form-control'>
                                    </div>
                                    <div class='col m-1'>
                                        <input type='number' value='{{old('to',$rate->to)}}' name='to[]' placeholder='{{trans('admin.range_to')}}' class='form-control'>
                                    </div>
                                    <div class='col m-1'>
                                        <input type='number' value='{{old('value',$rate->value)}}' name='value[]' placeholder='{{trans('admin.cost')}}' class='form-control'>
                                    </div>
                                </div>
                            </div>
                            @if(!$loop->first)<button class="list_del btn btn-danger"><i class='fa fa-trash'></i></button>@endif
                        </li>
                        @endforeach
                    </ol>
