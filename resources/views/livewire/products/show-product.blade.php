@php
$reviewCount = $this->product->ratings->count();
$product_rating_average = $this->product->ratings->avg('rating');
@endphp
    ...
    <div class="col-full">
        <div class="row">
            <nav class="woocommerce-breadcrumb" wire:ignore>
                ...
                @if(!empty($this->product->category->slug))
                <a
                    href="{{route('show_category',$this->product->category->slug)}}">{{$this->product->category->name}}</a>
                @endif
                <span class="delimiter">
                    <i class="tm tm-breadcrumbs-arrow-right"></i>
            ...
            <div class="col-md-12 mb-1 d-flex justify-content-center">
                <div class="row">
                        ...
                        <ul id="menu-primary-menu" class="nav yamm">
                            <li class="yamm-fw menu-item menu-item-has-children animate-dropdown dropdown">
                                <a title="Pages" data-toggle="dropdown" class="dropdown-toggle" aria-haspopup="true"
                                    href="#">{{$this->product->store->name}} <span
                                        class="caret {{($direction === 'right')?'mr-5':'ml-5'}}"></span></a>
                                <ul role="menu" class=" dropdown-menu">
                                    <li class="menu-item menu-item-object-static_block animate-dropdown" style="width: 450px;
                                    margin: auto;">
                                        <div class="yamm-content">
                                            <div class="tm-mega-menu">
                                                <div class="widget widget_nav_menu" style=' flex:none; max-width:none;'>
                                                    <ul class="menu" style="wrap:wrap;">
                                                        <li class="nav-body menu-item">
                                                            @if(isset($this->product->store->country->country_name))
                                                            <a
                                                                href="#">{{$this->product->store->country->country_name}}</a>
                                                            @endif
                                                        </li>
                                                        <li class="nav-body menu-item">
                                                            <a href="#">@lang('user.This_store_has_been_open_since')
                                                                <span
                                                                    class="text-danger">{{$this->product->store->created_at->format('F j, Y')}}</span></a>
                                                        </li>
                                                        <li class="nav-body menu-item row pl-3">
                                                            <a href="{{route('show_seller', (isset($this->product->store->slug))?$this->product->store->slug:1)}}" class="text-info col-md-4">@lang('user.Store_rating') </a>
                                                            <div class="rating-and-sharing-wrapper col-md-8">
                                                                <div class="woocommerce-product-rating">
                                                                    <div class="star-rating">
                                                                        <span
                                                                            style="width:{{$this->product->store->ratings->avg('rating') * 2 * 10}}%">Rated
                                                                    ...
                                                                    <a rel="nofollow"
                                                                        ...
                                                                        href="#reviews">(<span
                                                                            class="count">{{$this->product->store->ratings->count()}})</span>
                                                                        @lang('user.customer_review'))</a>
                                                                </div>
                                                            </div>
                                                        </li>
                                                        <li class="nav-body menu-item">
                                                            <a href="{{route('show_seller', (isset($this->product->store->slug))?$this->product->store->slug:1)}}"
                                                                class="text-info">@lang('user.See_the_store') <i
                                                                    class="fa fa-angle-double-{{($direction === 'right')?'left':'right'}}"></i></a>
                                                        </li>
                                                        <li class="nav-body menu-item">
                                                            <a href="{{route('show_chat', [
                                                                'memberTypeTo'   => 'seller',
                                                                'seq'            => $this->product->slug,
                                                                ])}}" class="text-info">@lang('user.Contact_now') <i
                    ...
                    <div id='follow'>
                        @guest
                        <a class="button {{($direction === 'right')?'mr-5':'ml-5'}} text-white" style="cursor:pointer;"
                            href="{{route('login')}}"><i class="fa fa-plus"></i> @lang('user.follow')</a>
                        @else
                        <a class="button {{($direction === 'right')?'mr-5':'ml-5'}} text-white {{($this->isFollow) ? 'disabled' : ''}}"
                        ...
                        @endguest
                        <h6 class="mt-1 font-weight-light text-right" style="color: #b8b8b8;">{{$store_followers_count}}
                            @lang('user.followers')</h6>
                    </div>
                </div>
            </div>
                        ...
                        <div class="single-product-wrapper">
                            <div class="product-images-wrapper thumb-count-4">
                                ...
                                <span class="onsale">
                                    <span class="woocommerce-Price-amount amount">
                                        {!! curr($this->product->calc_price() - $this->product->priceDiscount())
                                        !!}</span>
                                </span>
                                @endif
                                <!-- .onsale -->
                                <div id="techmarket-single-product-gallery"
                                    ...
                                    <div class="techmarket-single-product-gallery-thumbnails"
                                        ...
                                        <figure class="techmarket-single-product-gallery-thumbnails__wrapper">
                                            <figure data-thumb="{{Storage::url($this->product->image)}}"
                                            ...
                                            @foreach($this->product->gallery as $img)
                                            <figure data-thumb="{{Storage::url($img->file)}}"
                                                class="techmarket-wc-product-gallery__image">
                                                <img style="height:180px; width:180px;"
                                                    src="{{Storage::url($img->file)}}"
                                                    class="attachment-shop_thumbnail size-shop_thumbnail wp-post-image"
                            ...
                            <div class="summary entry-summary">
                                <div class="single-product-header">
                                    @php
                                    $discount_condition_buy_x = ($product->available_discount() &&
                                    $product->discount->condition === 'buy_x_and_get_y_free');
                                    @endphp
                                    ...
                                    @endphp
                                    <div class='row'>
                                        <h5 class='col-md-8' style='font-size:14px; margin-bottom:10px;'>
                                            @lang('user.buy')
                                    ...
                                    <a class='add_to_wishlist' @auth wire:click='wishlists({{$product->id}})' @else
                                ...
                                <div class="single-product-meta">
                                    ...
                                    <div class="brand">
                                        <a href="{{route('brand', $tradmark->slug)}}">
                                            <img alt="galaxy" src="{{Storage::url($tradmark->logo)}}">
                                        </a>
                                    </div>
                                    @endif
                                    <div class="cat-and-sku">
                                        @if(!empty($this->product->category->name))
                                    ...
                                    @if($product_rating_average > 4.5 && $product_rating_average < 5) <div
                                        class="product-label">
                                ...
                                @elseif($product_rating_average > 4 && $product_rating_average < 4.5) <div
                                    class="product-label">
                                    <div class="ribbon label green-label">
                                        <span>A</span>
                                    </div>
                        ...
                        <div class="rating-and-sharing-wrapper">
                            <div class="woocommerce-product-rating">
                                ...
                                <a rel="nofollow" class="woocommerce-review-link" href="#reviews">(<span
                                        class="count">{{$reviewCount}})</span> @lang('user.customer_review'))</a>
                            </div>
                        </div>
                    ...
                    <div class="techmarket-tabs techmarket-tabs-wrapper wc-tabs-wrapper">
                        ...
                        <div id="tab-reviews" class="techmarket-tab">
                            <div class="tab-content">
                                ...
                                <!-- /.ec-tabs -->
                                @livewire('products.add-reviews', ['item' => $this->product])
                            </div>
                            <!-- .tab-content -->
                        </div>
                        <!-- .techmarket-tab -->
