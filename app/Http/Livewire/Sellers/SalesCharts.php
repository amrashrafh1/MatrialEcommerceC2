...
class SalesCharts extends Component
{
    ...
    public function Revenue_Charts()
    {
        if ($this->profits === 'days') {
            ...
            $reveune->dataset('Renvenue', 'bar', revenue_chart(8, $this->profits, $this->store->id))->color('#0059bd')->backgroundColor('#0063d138');
            $reveune->dataset('Profit', 'bar', profit_chart(8, $this->profits, $this->store->id))->color('#0059bd')->backgroundColor('rgb(87, 170, 93,.6)');
            $reveune->dataset('cost', 'bar', cost_chart(8, $this->profits, $this->store->id))->color('#0059bd')->backgroundColor('rgb(220, 60, 69,.6)');
            return $reveune;
            ...
            $reveune = new SellerSalesChart;
            $reveune->labels(array_reverse($labels));
            $reveune->dataset('Renvenue', 'bar', revenue_chart(12, $this->profits, $this->store->id))->color('#0059bd')->backgroundColor('#0063d138');
            $reveune->dataset('Profit', 'bar', profit_chart(12, $this->profits, $this->store->id))->color('#0059bd')->backgroundColor('rgb(87, 170, 93,.6)');
            $reveune->dataset('cost', 'bar', cost_chart(12, $this->profits, $this->store->id))->color('#0059bd')->backgroundColor('rgb(220, 60, 69,.6)');
            return $reveune;
    ...
    public function Charts()
    {
        ...
        if ($this->period === 'days') {
            ...
            $chart->dataset('Sales', 'line', seller_sales_charts(8, $this->period, $this->store->id))->color('#0059bd')->backgroundColor('#0063d138');
            return $chart;
            ...
            $chart = new SellerSalesChart;
            $chart->labels($labels);
            $chart->dataset('Sales', 'line', seller_sales_charts(12, $this->period, $this->store->id))->color('#0059bd')->backgroundColor('#0063d138');
            return $chart;
            ...
            $chart = new SellerSalesChart;
            $chart->labels(array_reverse($labels));
            $chart->dataset('Sales', 'line', seller_sales_charts(12, $this->period, $this->store->id))
